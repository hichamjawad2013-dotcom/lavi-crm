...
const ModuleBiens = (() => {
            ...
            <tr data-bien-id="${b.ID}" style="cursor:pointer;">
              ...
              <td>
                <div class="td-actions">
                  ...
                  ${Auth.isAdmin() ? `<button class="btn-icon btn-sm" title="Supprimer" data-action="delete" data-id="${b.ID}" style="color:var(--danger);">
        ...
        <div>
          <div class="card" style="margin-bottom:16px;">
            ...
            <div class="card-body" style="display:flex; flex-direction:column; gap:8px; padding:14px;">
              ...
              <button class="btn btn-outline" id="btn-whatsapp-bien" style="justify-content:flex-start; font-size:13px; color:#1EBE5D; border-color:#1EBE5D;">
  ...
  async function _loadPlanSection(bien) {
    ...
    if (result.success) {
      if (_state.selectedId === bien.ID) _state.planUrl = result.url;
  ...
  function openEdit(id) {
    ...
    // Commercial : champs financiers (prix, commission) et affectation verrouillés
    // — le serveur les ignore de toute façon.
    if (!Auth.isAdmin()) {
      ['bien-prix', 'bien-commission', 'bien-commercial'].forEach(elId => {
        const el = document.getElementById(elId);
        if (el) { el.disabled = true; el.title = "Réservé à l'administrateur"; el.style.background = '#F2EFE9'; }
      });
    }
